...
Test(turn_left, succesful_left_facing_north)
{
    server_t server;
    zappy_t zappy;
    client_t client;
    ...
    int res = turn_left(&server, &zappy, NULL, &client);
    cr_assert_eq(res, EXIT_WITH_SUCCESS);
}

Test(turn_left, succesful_left_facing_south)
{
    server_t server;
    zappy_t zappy;
    client_t client;
