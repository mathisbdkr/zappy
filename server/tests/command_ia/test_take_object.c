...
Test(take_object, no_player)
{
    ...
    int res = take_object(&server, &zappy, NULL, &client);

    cr_assert_eq(res, EXIT_WITH_ERROR);
}

Test(take_object, all_good)
{
    ...
    client.client_fd = client_fd;
    server.cmd_arg_array = word_array("Take food\n", " \n");
    arg.width = 10;
    arg.height = 10;
...
Test(take_object, no_resource_on_land)
{
    zappy_t *zappy = malloc(sizeof(zappy_t));
    arguments_t arg;
    client_t client;
    ...
    client.client_fd = client_fd;
    server.cmd_arg_array = word_array("Take sibur\n", " \n");
    arg.width = 10;
    arg.height = 10;
